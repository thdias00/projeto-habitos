...
export default function MobileGroupVersion() {
    ...
    const { subscribe, unsubscribe } = useGroups();
    ...
    return (
        <ThemeProvider theme={theme}>
            <Box sx={{ pb: 7, display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
                <Grid container spacing={2} sx={{ display: "flex", justifyContent: "center" }}>
                    <Grid11>
                        <Box sx={{ borderRadius: "5px", background: `${theme.palette.secondary.main}`, height: "auto", padding: " 1rem 0.5rem", margin: "2rem" }}>
                            <Typography
                                color="black"
                                align="start"
                                component="p"
                            ...
                            {first &&
                                <>
                                    ...
                                    <div>
                                        {groups.group.users_on_group.includes((item) => item.id === user.id) ? <RemoveCircleIcon onClick={() => subscribe(groups.id)} /> : <AddCircleIcon onClick={() => unsubscribe(groups.id)} />}
                                    </div>
                                </>}
                        </Box>
                    </Grid11>
                    {first && <Grid11>
                        <Box sx={{ borderRadius: "5px", background: `${theme.palette.secondary.main}`, height: "auto", padding: " 1rem 0.5rem", margin: "2rem" }}>
                            {groups.group.users_on_group.map((item) =>
                                <div key={item.id} style={{ display: "flex", alignItems: "center", margin: "0.5rem" }} >
                                    <Avatar alt="User" src={IconUser} sx={{ width: "20%", height: "100%" }} />
                                    <div style={{ display: "flex", flexDirection: "column", margin: "0.5rem 1rem" }}>
                ...
                {second &&
                    <Grid container spacing={2} sx={{ display: "flex", justifyContent: "center" }}>
                        {activities.length > 0 ?
                            activities.map((item) =>
                                <Grid11 key={item.id}>
                                    <Box sx={{ borderRadius: "5px", background: `${theme.palette.secondary.main}`, height: "auto", padding: " 1rem 0.5rem", margin: "0.5rem 2rem" }}>
                                        ...
                                        <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }} >
                                            <div style={{ width: "100%" }}>
                                                <CardContent sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: "100%" }}>
                                                    <div>
                                                        <Typography sx={{ marginBottom: "1rem" }} variant="h6" component="div">
                                                            {item.title}
                                                        </Typography>
                                                    ...
                                                    <div style={{ display: "flex" }}>
                                                        ...
                                                        <ModalBase
                                                            ...
                                                            <GoalForm edit groupId={groups.group.id} goal={item} />
                                                        </ModalBase>
                                                    </div>
                                                </CardContent>
                                            </div>
                ...
                >
                    <BottomNavigation showLabels>
                        <BottomNavigationAction label="Participantes" icon={<GroupsIcon color={first ? "#000000" : "grey"} />} onClick={Groups} />
                        <BottomNavigationAction label="Atividades" icon={<ListAltIcon color={second ? "#000000" : "grey"} />} onClick={Activities} />
                        <BottomNavigationAction label="Metas" icon={<AssistantPhotoIcon color={third ? "#000000" : "grey"} />} onClick={Goals} />
                    </BottomNavigation>
                </Paper>
            </Box>
        </ThemeProvider >
